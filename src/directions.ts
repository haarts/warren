...
//
// A sheet gets its directions from two places: the four tips of its compass rose, and a short
// list of one-off bearings the rose does not cover. Everything below reads both through
// `directionsOf`, so the browser and the command line can never disagree about what is set.
import type { Pt } from './geom.ts'
import type { CompassRose, Direction, Sheet } from './model/types.ts'

/** A direction as found on a sheet, and where it came from. */
export interface SheetDirection extends Direction {
...
export function compassDirections(sheet: Sheet): SheetDirection[] {
  const rose = sheet.compass
  if (!rose) return []
  const out: SheetDirection[] = []
...
/** Every direction on the sheet: the rose's tips first, then the one-off list. */
export function directionsOf(sheet: Sheet): SheetDirection[] {
  return [
    ...compassDirections(sheet),
    ...(sheet.directions ?? []).map((d): SheetDirection => ({ ...d, source: 'list' })),
  ]
}
 ...
 */
export function resolveDirection(sheet: Sheet, term: string): SheetDirection | null {
  const list = directionsOf(sheet)
...
export function isToward(
  sheet: Sheet, from: Pt, to: Pt, term: string, toleranceDeg = 45,
): boolean | null {
  const dir = resolveDirection(sheet, term)
  if (!dir) return null
...
export function pointToward(sheet: Sheet, from: Pt, term: string, distance: number): Pt | null {
  const dir = resolveDirection(sheet, term)
  if (!dir) return null
